# -*- coding: utf-8 -*-
"""
Karnata — scripts/perfect_gold_ai_quality_guard.py
Fixes template substitution using lambda.
"""

with open("gold-rate.html", "r", encoding="utf-8") as f:
    html = f.read()

new_custom_ai_func = r"""    async function askCustomGoldAI() {
      ...
      // 1. Try Live Cloudflare Workers AI with Google Gemma Model
      try {
        ...
        if (response.ok) {
          ...
          if (resData && resData.success && resData.answer) {
            const rawAns = resData.answer.trim();
            // Quality Check: Verify it is pure, high quality Kannada without broken English phrases
            const hasBrokenPattern = rawAns.includes('(What will be') || rawAns.includes('ಇಂಟ್ರಾಜನ್') || rawAns.includes('ಅನುದಾನಿಕ ರಾಷ್ಟ್ರ');
            const hasKannada = /[\u0C80-\u0CFF]/.test(rawAns);

            if (!hasBrokenPattern && hasKannada && rawAns.length > 50) {
              badgeElem.textContent = '🤖 Google Gemma AI ವಿಶ್ಲೇಷಣೆ';
              badgeElem.style.background = '#DCFCE7';
              badgeElem.style.color = '#15803D';
              
              let formatted = rawAns
        ...
        console.warn('Real LLM call fallback to native engine:', e);
      }

      // 2. Client-Side Pristine Kannada High-Value Intelligence Engine
      // 1. UGADI (ಯುಗಾದಿ)
      ...
      // 3. FUTURE TARGETS (2027, 2028, 2030, ಭವಿಷ್ಯ)
      else if (q.includes('2027') || q.includes('2028') || q.includes('2030') || q.includes('ಭವಿಷ್ಯ') || q.includes('ಮುಂದಿನ ವರ್ಷ') || q.includes('future') || q.includes('ಎಷ್ಟು ದರ') || q.includes('ಎಷ್ಟು ಏರಬಹುದು')) {
        badgeElem.textContent = '🔮 2027 & 2030 ರ ದೀರ್ಘಾವಧಿ ಬೆಲೆ ಮುನ್ನೋಟ';
        badgeElem.style.background = '#FEF3C7';
        badgeElem.style.color = '#92400E';
        contentElem.innerHTML = `
          <strong>1. 10 ವರ್ಷಗಳ ಐತಿಹಾಸಿಕ CAGR ಬೆಳವಣಿಗೆ (+18.9% ವಾರ್ಷಿಕ):</strong><br>
          2016 ರಲ್ಲಿ 10 ಗ್ರಾಂ ಚಿನ್ನದ ಬೆಲೆ ₹28,623 ಇತ್ತು. ಇಂದು 2026 ರಲ್ಲಿ ₹1,63,040 ತಲುಪಿದೆ (5.6 ಪಟ್ಟು ಹೆಚ್ಚಳ). ಜಾಗತಿಕ ಹಣದುಬ್ಬರ, ಸೆಂಟ್ರಲ್ ಬ್ಯಾಂಕ್‌ಗಳ ನಿರಂತರ ಚಿನ್ನ ಸಂಗ್ರಹ ಮತ್ತು ರೂಪಾಯಿ ಮೌಲ್ಯದ ಸವಕಳಿಯ ಆಧಾರದ ಮೇಲೆ:<br><br>
          <strong>2. ನಿಖರ ಭವಿಷ್ಯದ ಬೆಲೆ ಗುರಿಗಳು (Future Target Projections):</strong><br>
          • <strong>2027 ರ ಸಂಭಾವ್ಯ ಗುರಿ (1 Year Target):</strong> 24K ಚಿನ್ನ <strong>₹18,500 ರಿಂದ ₹19,800 / ಗ್ರಾಂ</strong> (₹1.85 ಲಕ್ಷ - ₹1.98 ಲಕ್ಷ / 10 ಗ್ರಾಂ). 22K ಆಭರಣ ಚಿನ್ನ <strong>₹16,900 - ₹18,100 / ಗ್ರಾಂ</strong> (₹1,35,000 - ₹1,44,800 / 1 ಪವನ್).<br>
          • <strong>2030 ರ ದೀರ್ಘಾವಧಿ ಗುರಿ (5 Year Target):</strong> 24K ಚಿನ್ನ <strong>₹24,500 ರಿಂದ ₹27,500 / ಗ್ರಾಂ</strong> (₹2.45 ಲಕ್ಷ - ₹2.75 ಲಕ್ಷ / 10 ಗ್ರಾಂ). 22K ಆಭರಣ ಚಿನ್ನ <strong>₹22,400 - ₹25,200 / ಗ್ರಾಂ</strong> (₹1,79,000 - ₹2,01,600 / 1 ಪವನ್).<br><br>
          <strong>3. ಹೂಡಿಕೆದಾರರಿಗೆ AI ತಜ್ಞರ ತೀರ್ಮಾನ:</strong><br>
          ಭವಿಷ್ಯದ ಬೆಲೆ ಏರಿಕೆಯ ಲಾಭ ಪಡೆಯಲು ಒಟ್ಟಿಗೆ ದೊಡ್ಡ ಮೊತ್ತ ಹಾಕುವ ಬದಲು ಪ್ರತಿ ತಿಂಗಳು <strong>1 ಗ್ರಾಂ ನಿಂದ 2 ಗ್ರಾಂ ನಂತೆ ಡಿಜಿಟಲ್ ಗೋಲ್ಡ್ / SGB / Gold ETF ನಲ್ಲಿ ಹೂಡಿಕೆ ಮಾಡುತ್ತಾ ಬರುವುದು (SIP Method)</strong> ಅತ್ಯಂತ ಸುರಕ್ಷಿತ ಮತ್ತು ಗರಿಷ್ಠ ಸಂಪತ್ತು ಸೃಷ್ಟಿಸುವ ತಂತ್ರವಾಗಿದೆ.
        `;
      }
      // 4. AKSHAYA TRITIYA
...
html = re.sub(pattern, lambda m: new_custom_ai_func + "\n    function switchGoldTab", html)
...
print("SUCCESS_PERFECTED_AI_QUALITY_GUARD")
