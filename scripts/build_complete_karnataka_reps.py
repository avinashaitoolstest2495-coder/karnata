...
mlc_cats = [
    ('ವಿಧಾನಸಭೆಯಿಂದ ಆಯ್ಕೆ', tables_mlc[1]),
    ('ಸ್ಥಳೀಯ ಸಂಸ್ಥೆಗಳು', tables_mlc[2]),
    ('ಪದವೀಧರ ಕ್ಷೇತ್ರ', tables_mlc[3]),
    ('ಶಿಕ್ಷಕರ ಕ್ಷೇತ್ರ', tables_mlc[4]),
    ('ನಾಮನಿರ್ದೇಶನ', tables_mlc[5]),
]
...
for cat_label, tbl in mlc_cats:
    ...
    for r in rows:
        ...
        if len(cols) >= 3:
            ...
            const_name = cols[0] if not cols[0].isdigit() else cat_label
            party = 'INC'
            for c in cols:
...
# Update data/gis/representatives_catalog.json
catalog_path = 'data/gis/representatives_catalog.json'
with open(catalog_path, 'r', encoding='utf-8') as f:
    master_catalog = json.load(f)

master_catalog['total_mlcs'] = len(mlc_list)
master_catalog['total_rs'] = len(rs_list)
...
print("SUCCESS_UPDATED_REPRESENTATIVES_CATALOG_JSON")
