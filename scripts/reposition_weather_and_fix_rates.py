# -*- coding: utf-8 -*-
"""
Karnata — scripts/reposition_weather_and_fix_rates.py
1. Moves the creative Live District Weather + IMD NowCast Alert + 5-Day Forecast to the TOP of the main content (right under <main class="d-main">).
2. Removes any duplicated old weather boxes.
3. Fixes sidebar live rates across all 31 district files:
   - 24k / 22k Gold: ₹16,304 / ಗ್ರಾಂ (22K: ₹14,940/g)
   - Silver: ಬೆಳ್ಳಿ: ₹260.00/g
   - Petrol: ₹102.86 / L, Diesel: ₹88.94 / L
...
for dpath in district_files:
    ...
    # 1. Remove previous weather sections wherever they are
    html = re.sub(r'<!-- 🌦️ LIVE DISTRICT WEATHER[\s\S]*?</section>', '', html)
    # Remove old weather & dam dual grid if present
    html = re.sub(r'<!-- 3\. WEATHER & DAMS -->[\s\S]*?<section class="d-sec">\s*<div class="d-sec-title"><span>🌤️[^<]*ಹವಾಮಾನ ವರದಿ</span></div>[\s\S]*?</section>\s*(?=<section|</div>)', '', html)

    # 2. Build prominent top Weather & IMD Alert section
    ...
    <section style="background:#FFFFFF; border:1.5px solid #E2E8F0; border-radius:16px; padding:22px 24px; margin-bottom:24px; box-shadow:0 4px 16px rgba(0,0,0,0.04);">
...
"""
    ...
    # Insert top_weather_html right after `<main class="d-main">`
    if '<main class="d-main">' in html:
        html = html.replace('<main class="d-main">', '<main class="d-main">\n' + top_weather_html)

    # 3. Fix Sidebar Live Rates to 100% accurate 2026 figures
    ...
    html = re.sub(r'ಬೆಳ್ಳಿ:\s*₹?220/g', 'ಬೆಳ್ಳಿ: ₹260.00/g', html)
    html = re.sub(r'ಬೆಳ್ಳಿ:\s*₹?239\.90/g', 'ಬೆಳ್ಳಿ: ₹260.00/g', html)
...
print("SUCCESS_REPOSITIONED_WEATHER_AND_FIXED_RATES")
