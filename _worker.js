...
// Route: Official IMD Bengaluru District-Wise Nowcast & Warnings API (id=13)
    ...
    if (url.pathname.startsWith('/api/push/')) {
      ...
      if ((subPath === 'broadcast' || subPath === 'send') && request.method === 'POST') {
        ...
        // Static fallback to live_push_feed.json
        if (!feed.length && env && env.ASSETS) {
          try {
            const staticResp = await env.ASSETS.fetch(new Request('https://karnata.in/data/live_push_feed.json'));
            if (staticResp.ok) {
              const staticData = await staticResp.json();
              feed = staticData.feed || staticData || [];
            }
          } catch(e) {}
        }
...
async function handleQuizRequest(request, env) {
  ...
  // 1. Serve Universal Official Daily 20 Questions (Same for all users across Karnataka)
  try {
    const dailyFileUrl = (reqDate === todayStr)
      ? new URL('/data/daily_quiz.json', request.url)
      : new URL(`/data/quiz/quiz_${reqDate}.json`, request.url);

    const dRes = await env.ASSETS.fetch(new Request(dailyFileUrl));
    if (dRes.ok) {
      const dJson = await dRes.text();
      return new Response(dJson, { headers: corsHeaders });
    }
  } catch (e) {}

  // 2. Fallback to /data/daily_quiz.json
  try {
    const fallbackRes = await env.ASSETS.fetch(new Request(new URL('/data/daily_quiz.json', request.url)));
    if (fallbackRes.ok) {
      const fbJson = await fallbackRes.text();
      return new Response(fbJson, { headers: corsHeaders });
    }
  } catch (e) {}

  // 3. Fallback to Master Bank (First 20 Questions)
  try {
    ...
    if (bankRes.ok) {
      ...
      return new Response(JSON.stringify({
        success: true,
        date: reqDate,
        edition: "ಕರ್ನಾಟಕ ದೈನಂದಿನ ರಸಪ್ರಶ್ನೆ",
        total_questions: 20,
        questions: bankData.slice(0, 20)
      }), { headers: corsHeaders });
    }
  } catch (e) {}

  return new Response(JSON.stringify({ success: false, message: 'Daily quiz unavailable' }), {
    status: 500,
    headers: corsHeaders
  });
}
