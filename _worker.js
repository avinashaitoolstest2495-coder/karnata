    ...
    if (url.pathname === '/api/weather/nowcast' || url.pathname === '/api/imd-warnings') {
      ...
      try {
        ...
        if (staticRes.ok) {
          ...
          if (rawJson && rawJson.payload) {
            const baseWeatherData = decryptXorPayload(rawJson.payload, "NK_SECURE_KEY_2026_KARNATA");
            if (baseWeatherData && baseWeatherData.imd_warnings) {
              warningsMap = baseWeatherData.imd_warnings;
              updatedAt = baseWeatherData.updated_at || updatedAt;
            }
          }
        }
      } catch (e) {
      ...
      return new Response(JSON.stringify({
        success: true,
        source: 'IMD Bengaluru (Mausam Official)',
        updated_at: updatedAt,
        total_districts: Object.keys(warningsMap).length,
        districts: warningsMap
      }), { headers: corsHeaders });
    }
